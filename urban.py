...
def urban_search(original_search):
    ...
    # will write the html data to a file
    # with open('C:\\Users\\bmosier\\Desktop\\url_log.txt', 'w') as f:
        # for line in response:
            # f.write(line)

            # html += line
            
    # contains the html metadata
    html = response.read()

    # string to check if search item is defined
    not_defined = r"<i>" + re.escape(original_search)# + r"</i> isn\'t defined."
    ...
    if re.search(not_defined, html):
        ...
        if match:
            ...
            # #temp_list = temp.split('</div><div class=\"example\">')
            # definition = temp_list[0]
            # example = temp_list[1]
            # full_def =  original_search +": " + definition + ' EXAMPLE: ' + example
            # return full_def
            # # if example != '':
                # # print '\n' + example
        else:
